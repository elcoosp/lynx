...
public class LynxTemplateRender
  ...
  @CalledByNative
  static void onBTSHeapSnapshotResult(@Nullable Object callback, boolean success) {
    if (callback == null) {
      return;
    }
    TraceEvent.beginSection(TraceEventDef.LYNX_VIEW_TAKE_BTS_HEAP_SNAPSHOT_CALLBACK);
    try {
      ((LynxConsumer<Boolean>) callback).accept(success);
    } finally {
      TraceEvent.endSection(TraceEventDef.LYNX_VIEW_TAKE_BTS_HEAP_SNAPSHOT_CALLBACK);
    }
  }
