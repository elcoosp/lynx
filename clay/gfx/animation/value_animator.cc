...
bool ValueAnimator::ShouldReceiveAnimationFrame(int64_t current_time,
  ...
  if (end_listeners_called_ && HasFinishedAt(current_time) &&
      !(fill_mode_ & kForwards)) {
    if (next_lifecycle_time) {
      // Removing forwards fill from an ended animation must restore its
      // underlying value even while the animator is paused.
