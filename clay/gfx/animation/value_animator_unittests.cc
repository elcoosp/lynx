...
TEST(ValueAnimatorTest, ForwardsFillRetainsAnimatorUntilFillIsRemoved) {
  for (bool pause_before_removing_fill : {false, true}) {
    ...
    handler.SetCurrentTimeCallback([]() { return 32; });
    ...
    animator.SetFillMode(ValueAnimator::kNone);
    if (pause_before_removing_fill) {
      handler.RescheduleLifecycleCallback(32);
      ASSERT_FALSE(scheduled_delays.empty());
      EXPECT_EQ(scheduled_delays.back(), 0);
      handler.DoAnimationFrame(32, true);
    } else {
      handler.DoAnimationFrame(32);
    }
